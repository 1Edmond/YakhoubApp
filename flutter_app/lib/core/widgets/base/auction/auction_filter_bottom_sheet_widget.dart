...
class _AuctionFilterBottomSheetState extends State<AuctionFilterBottomSheet> {
  ...
  void _applyFilters() {
    if (_priceRange.start <= 0 || _priceRange.end <= 0) {
      showCustomSnackBarWidget(getTranslated('price_range_min_max_validation', context)!, context, isError: true);
      return;
    }
    if (_entryFeeRange.start <= 0 || _entryFeeRange.end <= 0) {
      showCustomSnackBarWidget(getTranslated('entry_fee_min_max_validation', context)!, context, isError: true);
      return;
    }
